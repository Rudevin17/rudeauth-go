# RudeAuth Go SDK: rules for an AI agent
...
Install: `go get github.com/Rudevin17/rudeauth-go`
...
Errors are sentinel values, matched with `errors.Is` (`rudeauth.ErrLicenseExpired`,
`rudeauth.ErrDeviceLimit`, `rudeauth.ErrDeviceBlacklisted`, and so on). `defer
sess.Close()` stops the session's heartbeat and zeroes the session key; call it as
soon as you hold a `*Session`.

## How to use this wrongly

Three real failure modes, in the SDK's own words:
